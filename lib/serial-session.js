...
class SerialSession extends EventEmitter {
  ...
  write(rawBytes) {
    ...
    const operation = async () => {
      ...
      // 仮想COMポートにはFlushFileBuffersを実装していないものがあり、drainだけが失敗する。
      // writeは完了しているので送信を失敗扱いにはせず、理由を1度だけ残して以後は省略する。
      // 送出完了を待たなくなるぶんACK待ちの開始がわずかに早まるが、
      // 送出できていない電文を「送信できた」と誤報告するよりは実害が小さい。
      if (this.drainSupported) {
        try {
          await callWithCallback(port, "drain");
        } catch (error) {
          this.drainSupported = false;
          this.emit("serial-error", {
            sessionId,
            message: "このポートはdrain(FlushFileBuffers)に対応していません。送信は完了しています。"
              + "以後drainを省略します: " + String(error && error.message || error),
          });
        }
      }
      const event = {
